-- Tutorial 14: clean the user-level sequence ODS into isolated DWD tables.

USE ecommerce_dwd;

CREATE EXTERNAL TABLE IF NOT EXISTS user_behavior_sequence_detail (
  user_id     BIGINT,
  item_id     BIGINT,
  category_id BIGINT,
  behavior    STRING,
...
STORED AS ORC
LOCATION '/warehouse/ecommerce/dwd/user_behavior_sequence_detail'
TBLPROPERTIES ('external.table.purge'='false');

CREATE EXTERNAL TABLE IF NOT EXISTS user_behavior_sequence_quarantine (
  user_id       BIGINT,
  item_id       BIGINT,
  category_id   BIGINT,
  behavior      STRING,
...
STORED AS ORC
LOCATION '/warehouse/ecommerce/dwd/user_behavior_sequence_quarantine'
TBLPROPERTIES ('external.table.purge'='false');

SET hive.exec.dynamic.partition=true;
SET hive.exec.dynamic.partition.mode=nonstrict;

-- Previous UTC-based builds could register this invalid Shanghai date partition.
ALTER TABLE user_behavior_sequence_detail DROP IF EXISTS PARTITION (dt='2017-11-24');

INSERT OVERWRITE TABLE user_behavior_sequence_detail PARTITION (dt)
SELECT
  user_id,
  item_id,
  category_id,
...
INSERT OVERWRITE TABLE user_behavior_sequence_quarantine
SELECT
  user_id,
  item_id,
  category_id,
